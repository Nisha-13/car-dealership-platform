<template>
  <div>
    ...
    <div class="admin-dash-grid">
      <!-- Quick Inventory & Operations Summary -->
      <div class="glass-panel" style="padding:1.5rem;">
        ...
        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(130px, 1fr)); gap:1rem; margin-bottom:1.5rem; text-align:center;">
          <div style="background:rgba(255,255,255,0.03); padding:1rem; border-radius:8px; border:1px solid var(--border-color);">
            ...
            <div style="font-size:1.4rem; font-weight:800; color:var(--text-main);">{{ stats.totalCars || 0 }}</div>
          </div>

          <div style="background:rgba(255,255,255,0.03); padding:1rem; border-radius:8px; border:1px solid var(--border-color);">
            <div style="font-size:0.8rem; color:var(--text-muted);">Reserved Cars</div>
            <div style="font-size:1.4rem; font-weight:800; color:#f59e0b;">{{ stats.reservedCars || 0 }}</div>
          </div>

          <div style="background:rgba(255,255,255,0.03); padding:1rem; border-radius:8px; border:1px solid var(--border-color);">
            <div style="font-size:0.8rem; color:var(--text-muted);">Sold Vehicles</div>
            <div style="font-size:1.4rem; font-weight:800; color:var(--status-confirmed);">{{ stats.soldCars || 0 }}</div>
          </div>

          <div style="background:rgba(255,255,255,0.03); padding:1rem; border-radius:8px; border:1px solid var(--border-color);">
            <div style="font-size:0.8rem; color:var(--text-muted);">Registered Clients</div>
            <div style="font-size:1.4rem; font-weight:800; color:var(--primary);">{{ stats.totalCustomers || 0 }}</div>
          </div>
        </div>

        <h4 style="font-family:var(--font-heading); font-size:1.05rem; font-weight:700; margin-bottom:1rem;">Operational Supervision Shortcuts</h4>
        <div style="display:flex; gap:0.75rem; flex-wrap:wrap;">
          <router-link to="/admin/reservations" class="btn btn-primary btn-sm">🚗 Manage Reservations</router-link>
          <router-link to="/admin/test-drives" class="btn btn-secondary btn-sm">📅 Test Drive Bookings</router-link>
          <router-link to="/admin/inquiries" class="btn btn-secondary btn-sm">💬 Review Inquiries</router-link>
          <router-link to="/admin/messages" class="btn btn-accent btn-sm">✉️ Support Console</router-link>
        </div>
      </div>
